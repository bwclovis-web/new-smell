...
const saveReports = async (reportsDir, timestamp, missingData, duplicates, housesWithNoPerfumes) => {
  try {
    ...
    if (missingData) {
      ...
      let mdContent = `# Missing Information Report\n\nGenerated: ${new Date().toLocaleString()}\n\nTotal perfumes with missing info: ${missingData.length}\n\n`
      mdContent += '## By Brand\n\n'
    ...
    if (housesWithNoPerfumes) {
      const housesPath = path.join(reportsDir, `houses_no_perfumes_${timestamp}.json`)
      ...
      const housesMdPath = path.join(reportsDir, `houses_no_perfumes_${timestamp}.md`)
...
const main = async () => {
  try {
    ...
    await saveReports(reportsDir, timestamp, missingData, duplicates, housesWithNoPerfumes)
