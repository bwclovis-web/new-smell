...
function showUsage() {
  log('\n📖 Usage:', 'bright');
  log('node push_local_to_remote_db_cli.js <remote_LOCAL_DATABASE_URL> [--keep-remote]', 'cyan');
  log('\n📝 Examples:', 'bright');
  log('node push_local_to_remote_db_cli.js "postgresql://user:pass@host:5432/db"', 'yellow');
  log('node push_local_to_remote_db_cli.js "postgresql://user:pass@host:5432/db" --keep-remote', 'yellow');
  log('\n🔧 Options:', 'bright');
  log('--keep-remote    Keep the remote database configuration after pushing', 'yellow');
