...
function mapHouseType(type) {
  if (!type) return 'indie'
  
  const typeLower = type.toLowerCase()
  if (typeLower.includes('niche')) return 'niche'
  if (typeLower.includes('designer')) return 'designer'
  if (typeLower.includes('celebrity')) return 'celebrity'
  if (typeLower.includes('drugstore')) return 'drugstore'
...
if (process.argv[1] && process.argv[1].endsWith('import_s_and_s_complete.js')) {
  main()
}
