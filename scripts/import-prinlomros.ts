// Import script for perfumes_prinlomros.csv from csv directory
...
function calculateDataCompleteness(data: any): number {
  let score = 0
  if (data.description && data.description.trim() && data.description.trim() !== "not available at this time") {
    score += 10
  }
  if (data.image && data.image.trim() && !data.image.includes("placeholder")) {
    score += 10
  }
...
function parseNotes(notesString: string): string[] {
  ...
  try {
    ...
    return Array.isArray(parsed) ? parsed.map(n => n.trim().toLowerCase()).filter(n => n.length > 0) : []
  } catch {
...
async function getOrCreateNote(noteName: string) {
  ...
  if (!note) {
    ...
    console.log(`  ♻️  Using existing note: ${trimmedNoteName}`)
  }
...
async function importPerfumeData(csvFiles: string[]) {
  ...
  for (const csvFile of csvFiles) {
    ...
    await importCsv(filePath, async data => {
      ...
      if (existingPerfumes.length > 0) {
        ...
        if (sameHousePerfumes.length > 0) {
          ...
          if (newScore > bestExisting.score) {
            ...
            if ((!bestExisting.perfume.description || bestExisting.perfume.description === "not available at this time") && data.description && data.description !== "not available at this time") {
              updateData.description = data.description.trim() || null
            }
            if ((!bestExisting.perfume.image || bestExisting.perfume.image.includes("placeholder")) && data.image && !data.image.includes("placeholder")) {
        ...
        created++
        console.log(`✅ Created: ${perfumeName}`)
      }
      ...
      // Delete existing note relations to replace with new ones
      await prisma.perfumeNoteRelation.deleteMany({
        where: { perfumeId: perfume.id },
      })

      // Create note relations using existing notes (only create new notes when needed)
...
async function main() {
  console.log("🚀 Starting CSV import for Prinlomros Parfum Prissana...")
  
  const csvFiles = ["perfumes_prinlomros.csv"]
