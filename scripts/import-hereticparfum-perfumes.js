#!/usr/bin/env node
/**
 * Import Heretic Parfum Perfumes to database
...
async function createOrGetPerfumeHouse(houseName) {
  ...
  return await prisma.perfumeHouse.create({
    data: {
      ...
      type: "indie", // Heretic Parfum is an indie house
    },
  })
}
...
async function importHereticParfumPerfumesData() {
  const csvFile = "perfumes_hereticparfum.csv"
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    let slugCounter = 1
    while (true) {
      const existingSlug = await prisma.perfume.findUnique({
...
// Run the import
importHereticParfumPerfumesData()
  .then(async () => {
