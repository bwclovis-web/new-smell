...
async function testCompression(endpoint) {
  ...
  try {
    ...
    const meetsExpectation = endpoint.expectedCompression ? isCompressed : !isCompressed
    const status = meetsExpectation ? '✅' : '❌'
    
    console.log(`${status} Compression Status: ${isCompressed ? 'COMPRESSED' : 'NOT COMPRESSED'}`)
...
async function runCompressionTests() {
  ...
  const avgCompressionRatio = results
  ...
  results.forEach(result => {
    ...
    const compression = result.isCompressed ? `(${result.compressionRatio}%)` : '(not compressed)'
