...
const result = await retryFetch(() => fetch("/api/data").then((r) => r.json()), {
  maxAttempts: 3,
  initialDelay: 1000,
})
...
const { initialData, isLoading, error, loadDataByLetter } = useDataByLetter({
  endpoint: "/api/houses",
  itemName: "houses",
})
```
...
const { perfumes, loading, hasMore, loadMorePerfumes } = useInfiniteScroll({
  houseSlug,
  initialPerfumes,
  scrollContainerRef,
  take: 9,
...
const { searchValue, setSearchValue, results, isLoading } = useDebouncedSearch(
