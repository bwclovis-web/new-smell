...
**File**: `app/utils/server/user-limit.server.ts` (new)
...
**File**: `app/utils/server/stripe.server.ts` (new)
...
- [x] **2.1** Add Stripe env to validation
  - [x] In `app/utils/security/env.server.ts`: add `STRIPE_SECRET_KEY`, `STRIPE_PUBLISHABLE_KEY`, `STRIPE_WEBHOOK_SECRET` (required in production)
  - [x] In `.env.example` or `env_example.txt`: document the three Stripe variables
- [x] **2.2** Install Stripe
  - [x] `npm install stripe`
  - [x] Add types if needed: `npm install -D @types/stripe` (only if not included) — not needed; Stripe ships with TypeScript types

### Phase 3: User limit & signup gating

- [x] **3.1** Create user-limit utility
  - [x] Create `app/utils/server/user-limit.server.ts`
  - [x] Export `FREE_USER_LIMIT = 100`
  - [x] Implement `getCurrentUserCount()` (count users where `isEarlyAdopter === true` OR `subscriptionStatus === 'paid'`)
  - [x] Implement `canSignupForFree(): Promise<boolean>`
- [x] **3.2** Update user model for signup
  - [x] In `app/models/user.server.ts`: update `createUser()` to accept optional `subscriptionStatus` and set `isEarlyAdopter` based on count (or caller passes it)
- [x] **3.3** Gate signup action
  - [x] In signup route (e.g. `app/routes/login/SignUpPage.tsx` or where signup action lives): at start of action, call `canSignupForFree()`
  - [x] If `false`, redirect to `/subscribe?redirect=/sign-up` (or your signup path)
  - [x] If `true`, create user with `isEarlyAdopter: true` and `subscriptionStatus: 'free'`

### Phase 4: Stripe server utilities

- [x] **4.1** Create Stripe server module
  - [x] Create `app/utils/server/stripe.server.ts`
  - [x] Initialize Stripe with `STRIPE_SECRET_KEY`
  - [x] Implement `createCheckoutSession()` (e.g. for subscription product/price IDs from env or config)
  - [x] Implement `getSubscriptionStatus()` (optional; for looking up by subscription ID or customer ID)
  - [x] Add JSDoc or comments for required env vars

### Phase 5: Subscribe page & checkout UI

- [x] **5.1** Subscription page route
  - [x] Create `app/routes/subscribe.tsx`
  - [x] Read `redirect` query param (e.g. `/sign-up`)
  - [x] Show copy explaining 100-user limit and that payment is required to sign up
  - [x] Add CTA that starts Stripe Checkout (or embed Stripe Elements)
...
- [x] **6.1** Stripe webhook route
  - [x] Create `app/routes/api/stripe-webhook.ts` (no UI)
  - [x] Verify webhook signature using `STRIPE_WEBHOOK_SECRET`
  - [x] Handle `checkout.session.completed`: set user subscription (by email) — update user with `subscriptionStatus: 'paid'`, `subscriptionId`, `subscriptionStartDate`
  - [x] Handle `customer.subscription.updated` and `customer.subscription.deleted` (update or set `subscriptionStatus: 'cancelled'`)
  - [x] Return 200 quickly; handler does minimal DB work then returns
- [x] **6.2** Subscribe-success route
  - [x] Create `app/routes/subscribe-success.tsx`
  - [x] Read session or token from query/Stripe; show success message
  - [x] Redirect to signup with pre-filled email if possible, or link to signup; ensure new user is created with `subscriptionStatus: 'paid'`

### Phase 7: Queries & polish

- [x] **7.1** User queries
  - [x] In `app/models/user.query.ts`: added `getUsersBySubscriptionStatus(status)`, `getUsersByEarlyAdopter(isEarlyAdopter)`, `getUserBySubscriptionId(subscriptionId)` for admin and gating
