...
describe('Error Handling Performance', () => {
  ...
  describe('Error Creation Overhead', () => {
    it('should create AppError instances with minimal overhead', () => {
      ...
      // Average per error should be < 0.01ms
    ...
    it('should create errors with all types efficiently', () => {
      ...
      const duration = global.measurePerformance('Create all error types 100x', () => {
        for (let i = 0; i < 100; i++) {
          errorTypes.forEach(createFn => createFn())
        }
      })

      // 900 error creations (9 types x 100) should take < 10ms
      expect(duration).toBeLessThan(10)
    })
  })
  ...
  describe('Error Handler Wrapper Overhead', () => {
    it('should add minimal overhead to successful operations', async () => {
      ...
      const wrappedFn = withErrorHandler(successfulFn)

      const duration = global.measurePerformance('Wrapped function calls (1000x)', async () => {
        const promises = []
      ...
      // 1000 wrapped calls should complete in < 50ms
    ...
    it('should handle errors with acceptable overhead', async () => {
      ...
      const wrappedFn = withErrorHandler(errorFn)

      const duration = global.measurePerformance('Error handling (100x)', async () => {
        const promises = []
        for (let i = 0; i < 100; i++) {
    ...
    it('should not accumulate overhead with nested wrappers', async () => {
      ...
      // Wrap multiple times
      const wrapped1 = withErrorHandler(baseFn)
      const wrapped2 = withErrorHandler(wrapped1)
      const wrapped3 = withErrorHandler(wrapped2)

      const duration = global.measurePerformance('Nested wrappers (1000x)', async () => {
        const promises = []
        for (let i = 0; i < 1000; i++) {
  ...
  describe('Retry Mechanism Performance', () => {
    it('should have minimal overhead for successful operations (no retries)', async () => {
      ...
      const duration = global.measurePerformance('Retry wrapper success (1000x)', async () => {
        const promises = []
        for (let i = 0; i < 1000; i++) {
    ...
    it('should handle retries efficiently with fake timers', async () => {
      ...
      // With fake timers, the operation should complete very quickly
      // (actual delays are mocked)
      expect(duration).toBeLessThan(50)
  ...
  describe('ErrorLogger Performance', () => {
    it('should log errors efficiently', () => {
      ...
      const duration = global.measurePerformance('Log 1000 errors', () => {
        for (let i = 0; i < 1000; i++) {
          logger.logError(
            createError.validation(`Test error ${i}`),
            { userId: 'test-user', context: { iteration: i } }
          )
        }
      })

      // Logging 1000 errors should take < 50ms
      expect(duration).toBeLessThan(50)
    ...
    it('should retrieve logs efficiently', () => {
      ...
      // 100 log retrievals should be very fast (< 10ms)
      expect(duration).toBeLessThan(10)
    })
    ...
    it('should filter logs efficiently', () => {
      ...
      // Populate with different error types
      for (let i = 0; i < 100; i++) {
        logger.logError(createError.validation('Validation'), { userId: 'user-1' })
        logger.logError(createError.network('Network'), { userId: 'user-2' })
        logger.logError(createError.server('Server'), { userId: 'user-3' })
      }

      const duration = global.measurePerformance('Filter logs by user 100x', () => {
        for (let i = 0; i < 100; i++) {
          logger.getLogs('user-1')
        }
      })

      // 100 filtered retrievals should be fast (< 20ms)
    ...
    it('should clear logs efficiently', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        logger.logError(createError.validation(`Test ${i}`), {})
      }
  ...
  describe('Overall Error Handling Overhead', () => {
    it('should maintain < 100ms overhead for typical error handling flow', async () => {
      ...
      const typicalFlow = async (id: number) => {
        try {
          ...
          // Log error
          logger.logError(appError, { userId: 'test-user', context: { id } })
      ...
      const wrappedFlow = withErrorHandler(typicalFlow)

      const duration = global.measurePerformance('Typical error flow (100x)', async () => {
        const promises = []
        for (let i = 0; i < 100; i++) {
    ...
    it('should handle high-frequency errors without performance degradation', async () => {
      ...
      const wrappedOp = withErrorHandler(highFrequencyOperation)

      const duration = global.measurePerformance('High frequency errors (1000x)', async () => {
        const promises = []
        for (let i = 0; i < 1000; i++) {
      ...
      // Even with 1000 errors, should complete in < 100ms
      expect(duration).toBeLessThan(100)
    ...
    it('should maintain performance under concurrent load', async () => {
      ...
      const wrappedOp = withErrorHandler(operation)

      const duration = global.measurePerformance('Concurrent operations (500x)', async () => {
        const promises = []
        for (let i = 0; i < 500; i++) {
      ...
      // 500 concurrent operations (mix of success/failure) should be < 100ms
      expect(duration).toBeLessThan(100)
    })
  })
  ...
  describe('Error Sanitization Overhead', () => {
    it('should sanitize errors with minimal overhead', () => {
      ...
      // Sanitization should add minimal overhead (< 50ms for 1000 errors)
      expect(duration).toBeLessThan(50)
    })
  })
  ...
  describe('Performance Benchmarks Summary', () => {
    it('should meet all performance requirements', async () => {
      ...
      // Test error creation
      results.errorCreation = global.measurePerformance('Error creation (100x)', () => {
        for (let i = 0; i < 100; i++) {
      ...
      const wrapped = withErrorHandler(errorFn)
      
      results.errorHandling = global.measurePerformance('Error handling (100x)', async () => {
        const promises = []
        for (let i = 0; i < 100; i++) {
      ...
      results.retryMechanism = global.measurePerformance('Retry mechanism (100x)', async () => {
        const promises = []
        for (let i = 0; i < 100; i++) {
      ...
      results.errorLogging = global.measurePerformance('Error logging (100x)', () => {
        for (let i = 0; i < 100; i++) {
          logger.logError(createError.validation(`Test ${i}`), {})
        }
      })
      ...
      const typicalOp = withErrorHandler(async () => {
        if (Math.random() > 0.5) {
      ...
      results.overallOverhead = global.measurePerformance('Overall overhead (100x)', async () => {
        const promises = []
        for (let i = 0; i < 100; i++) {
