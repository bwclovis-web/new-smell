...
export const testTableSorting = async (
  ...
  expect(header).toHaveAttribute('aria-sort', expectedOrder === 'asc' ? 'ascending' : 'descending')
}
...
export const testDataExport = async (
  ...
  const user = userEvent.setup()
  const exportButton = screen.getByRole('button', { name: new RegExp(`export.*${exportFormat}`, 'i') })
...
export const testColumnVisibility = async (
  ...
  const table = screen.getByRole('table')
  const header = within(table).queryByRole('columnheader', { name: columnName })
