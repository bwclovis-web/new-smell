...
test.describe('User Profile & Preferences', () => {
  test.describe('Profile Management', () => {
    ...
    test('should require current password to change password', async ({
      page,
    }) => {
      await page.goto('/profile')
  ...
  test.describe('Login Security', () => {
    ...
    test('should not expose whether email exists', async ({ page }) => {
      ...
      const error1 = await page.locator('[data-testid="error-message"]').textContent()

      // Try existing email with wrong password
      await loginPage.navigateTo()
      await loginPage.login('test@example.com', 'wrongpassword')
      const error2 = await page.locator('[data-testid="error-message"]').textContent()
