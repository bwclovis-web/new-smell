...
describe('ErrorDisplay', () => {
  ...
  describe('Actions', () => {
    ...
    it('should not render action buttons when no actions provided', () => {
      const error = createError.validation('Validation error', 'VALIDATION_MISSING_FIELD')
      render(<RouterWrapper>
          <ErrorDisplay error={error} />
        </RouterWrapper>)
  ...
  describe('Technical Details', () => {
    ...
    it('should display error context when present', () => {
      const error = createError.server('Test error', 'SERVER_ERROR', { userId: '123', action: 'test' })
      const { container } = render(<RouterWrapper>
          <ErrorDisplay error={error} showDetails={true} />
        </RouterWrapper>)
  ...
  describe('Accessibility', () => {
    ...
    it('should have aria-label on dismiss button', () => {
      ...
      const dismissButton = screen.getByRole('button', { name: /dismiss this error message/i })
      expect(dismissButton).toHaveAttribute('aria-label', 'Dismiss this error message')
    })
