...
describe('createValidator', () => {
  ...
  it('should return null when all validations pass', () => {
    ...
    const errors = validator({ email: 'test@example.com', password: 'password123' })
    expect(errors).toBe(null)
  })

  it('should provide access to all values in validator functions', () => {
    const validator = createValidator<{ password: string; confirmPassword: string }>({
...
describe('sanitizeFormInput', () => {
  ...
  it('should remove angle brackets', () => {
    expect(sanitizeFormInput('<script>alert("xss")</script>'))
      .toBe('scriptalert("xss")/script')
  })

  it('should remove javascript: protocol', () => {
    expect(sanitizeFormInput('javascript:alert("xss")'))
      .toBe('alert("xss")')
  })

  it('should remove event handlers', () => {
    expect(sanitizeFormInput('onclick=alert("xss")'))
      .toBe('alert("xss")')
  })

  it('should handle normal input', () => {
    expect(sanitizeFormInput('Normal text input'))
      .toBe('Normal text input')
  })
})
...
describe('VALIDATION_MESSAGES', () => {
  it('should have correct message templates', () => {
    ...
    expect(VALIDATION_MESSAGES.minLength('Password', 8))
      .toBe('Password must be at least 8 characters')
    expect(VALIDATION_MESSAGES.maxLength('Username', 20))
      .toBe('Username must be at most 20 characters')
    expect(VALIDATION_MESSAGES.match('Password', 'Confirm Password'))
      .toBe('Password and Confirm Password must match')
  })
})
