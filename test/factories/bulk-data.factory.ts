...
export function generateBulkTestData(config: BulkDataConfig = {}) {
  ...
  const perfumes = houses.flatMap((house, houseIndex) =>
    createMockPerfumesForHouse(house.id, perfumesPerHouse).map((perfume, perfumeIndex) => ({
      ...perfume,
  ...
  const userCollections = users.slice(0, Math.ceil(userCount / 2)).map((user, userIndex) => {
