...
const extendedSchema = z.object({
  ...
  METRICS_PORT: z
    .string()
    .regex(/^\d+$/, "METRICS_PORT must be a valid port number")
    .optional(),
  APP_PORT: z
    .string()
    .regex(/^\d+$/, "APP_PORT must be a valid port number")
    .optional(),

  // Database configuration
  LOCAL_DATABASE_URL: z
    .string()
    .url("LOCAL_DATABASE_URL must be a valid URL")
    .optional(),
  ...
  // Security features
  HONEYPOT_ENCRYPTION_SEED: z
    .string()
    .min(16, "HONEYPOT_ENCRYPTION_SEED must be at least 16 characters")
    .optional(),
  ...
  REDIS_PORT: z
    .string()
    .regex(/^\d+$/, "REDIS_PORT must be a valid port number")
    .optional(),
  ...
  TEST_DATABASE_URL: z
    .string()
    .url("TEST_DATABASE_URL must be a valid URL")
    .optional(),
