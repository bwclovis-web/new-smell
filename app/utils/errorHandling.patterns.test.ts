...
describe('errorHandling.patterns', () => {
  ...
  describe('withValidationErrorHandling', () => {
    it('should return result on success', () => {
      const result = withValidationErrorHandling(
        () => ({ valid: true }),
        { schema: 'testSchema' }
      )
  ...
  describe('assertion helpers', () => {
    describe('assertExists', () => {
      ...
      it('should throw notFoundError if value is null', () => {
        expect(() => assertExists(null, 'User', { userId: '123' }))
          .toThrow(AppError)
      })

      it('should throw notFoundError if value is undefined', () => {
        expect(() => assertExists(undefined, 'User', { userId: '123' }))
          .toThrow(AppError)
      })
    })
    ...
    describe('assertValid', () => {
      ...
      it('should throw validationError if condition is false', () => {
        expect(() => assertValid(false, 'Invalid', { field: 'email' }))
          .toThrow(AppError)
      })
    })
    ...
    describe('assertAuthenticated', () => {
      ...
      it('should throw authenticationError if userId is null', () => {
        expect(() => assertAuthenticated(null, { route: '/admin' }))
          .toThrow(AppError)
      })

      it('should throw authenticationError if userId is undefined', () => {
        expect(() => assertAuthenticated(undefined, { route: '/admin' }))
          .toThrow(AppError)
      })
    })
    ...
    describe('assertAuthorized', () => {
      ...
      it('should throw authorizationError if condition is false', () => {
        expect(() => assertAuthorized(false, 'Not authorized', { userId: '123' }))
          .toThrow(AppError)
      })
    })
  })
