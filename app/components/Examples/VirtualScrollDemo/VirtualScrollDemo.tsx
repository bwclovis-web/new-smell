...
const VirtualScrollDemo: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Virtual Scrolling Demo</h1>
      ...
      <div className="bg-blue-50 p-4 rounded-lg mb-6">
        <h3 className="text-lg font-semibold text-blue-800 mb-2">Performance Information</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div>
            <span className="font-medium">Total Items:</span> {itemCount.toLocaleString()}
          </div>
          <div>
            <span className="font-medium">Visible Items:</span> {Math.ceil(containerHeight / itemHeight)}
          </div>
          <div>
            <span className="font-medium">Rendered Items:</span> {useVirtualScrolling ? Math.ceil(containerHeight / itemHeight) + 10 : itemCount}
          </div>
        </div>
      </div>
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div className="bg-gray-100 px-4 py-2 border-b">
          <h3 className="font-semibold text-gray-700">
            {useVirtualScrolling ? 'Virtual Scrolling' : 'Regular Rendering'} - {itemCount} items
          </h3>
        </div>
        ...
        {useVirtualScrolling ? (
          <VirtualScrollList
            ...
            itemClassName="mb-4"
            emptyState={<div className="p-8 text-center text-gray-500">No items to display</div>}
          />
        ) : (
          <div
      ...
      <div className="mt-6 bg-yellow-50 p-4 rounded-lg">
        ...
        <ul className="text-sm text-yellow-700 space-y-1">
          <li>• Adjust the sliders to change the number of items, container height, and item height</li>
          <li>• Toggle virtual scrolling on/off to see the performance difference</li>
          <li>• With virtual scrolling enabled, only visible items are rendered in the DOM</li>
          <li>• Try scrolling with large datasets to see the smooth performance</li>
          <li>• Notice how the "Rendered Items" count stays low with virtual scrolling</li>
        </ul>
      </div>
    </div>
  )
