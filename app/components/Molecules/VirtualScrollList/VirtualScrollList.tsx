...
export interface VirtualScrollListProps extends Omit<VirtualScrollProps, 'children'> {
...
const VirtualScrollList: React.FC<VirtualScrollListProps> = ({
  ...
  if (!hasItems && emptyState) {
    return <div className={styleMerge('flex items-center justify-center h-full', className)}>{emptyState}</div>
  }

  if (!hasItems) {
    return (
      <div className={styleMerge('flex items-center justify-center h-full text-gray-500', className)}>
        No items to display
      </div>
    )
  }
