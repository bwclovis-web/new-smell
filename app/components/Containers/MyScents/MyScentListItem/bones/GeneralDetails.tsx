    ...
    <div className="flex gap-10 mt-6 justify-between items-center">
      {userPerfume.placeOfPurchase &&
        <p className="font-medium flex flex-col justify-start items-start">
          ...
          <span className='text-2xl text-noir-gold-100 capitalize'>{userPerfume.placeOfPurchase}</span>
        </p>
      }
      <div className="flex items-start justify-start gap-8">
        {userPerfume.price &&
          <p className='flex flex-col items-end justify-start'>
            <span className='text-lg font-medium'>{t('myScents.listItem.price')}</span>
            <span className='text-xl text-noir-gold-100'>{formatPrice(userPerfume.price)}</span>
          </p>
        }
        <p className='flex flex-col items-end justify-start'>
          <span className='text-lg font-medium'>{t('myScents.listItem.type')}</span>
          <span className='text-xl text-noir-gold-100'>{getPerfumeTypeLabel(userPerfume.type)}</span>
        </p>
      </div>
