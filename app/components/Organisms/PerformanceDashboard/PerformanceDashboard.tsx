  ...
  const collectPerformanceData = useCallback(() => {
    ...
    try {
      ...
      const resourceMetrics = {
        ...
        loadTime: resources.reduce((total, resource) => total + resource.duration, 0)
      }
      ...
      if (resourceMetrics.totalSize > 2 * 1024 * 1024) { // 2MB
    ...
    <div className={styleMerge('bg-white border border-gray-200 rounded-lg shadow-lg p-6 max-w-4xl', className)}>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-2">
          <div className={`w-3 h-3 rounded-full ${isCollecting ? 'bg-yellow-400 animate-pulse' : 'bg-green-400'}`} />
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className={`text-sm ${getScoreColor(getPerformanceScore(performanceData.navigation.loadComplete, thresholds.lcp))}`}>
              {getPerformanceScore(performanceData.navigation.loadComplete, thresholds.lcp).replace('-', ' ')}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className={`text-sm ${getScoreColor(getPerformanceScore(performanceData.navigation.domContentLoaded, thresholds.tti))}`}>
              {getPerformanceScore(performanceData.navigation.domContentLoaded, thresholds.tti).replace('-', ' ')}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className={`text-sm ${getScoreColor(getPerformanceScore(performanceData.navigation.domContentLoaded, thresholds.fcp))}`}>
              {getPerformanceScore(performanceData.navigation.domContentLoaded, thresholds.fcp).replace('-', ' ')}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className={`text-sm ${getScoreColor(getPerformanceScore(performanceData.webVitals.cls, thresholds.cls, true))}`}>
              {getPerformanceScore(performanceData.webVitals.cls, thresholds.cls, true).replace('-', ' ')}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Navigation Timing */}
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Navigation Timing</h3>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm font-semibold text-gray-800">
                {formatBytes(performanceData.memory.used)} / {formatBytes(performanceData.memory.limit)}
              </span>
            </div>
      ...
      {alerts.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Performance Alerts</h3>
