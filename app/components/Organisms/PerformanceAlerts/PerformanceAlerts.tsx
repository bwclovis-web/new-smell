        ...
        rules.forEach(rule => {
          ...
          switch (rule.metric) {
            ...
            case 'resourceCount':
              ...
              break
            case 'bundleSize':
              value = performance.getEntriesByType('resource').reduce((total, resource) => total + (resource.transferSize || 0), 0)
              break
            case 'memoryUsage':
              if ((performance as any).memory) {
                value = (performance as any).memory.usedJSHeapSize / (performance as any).memory.jsHeapSizeLimit
              }
              break
          }
  ...
  return (
    <div className={styleMerge('bg-white border border-gray-200 rounded-lg shadow-lg p-6', className)}>
      ...
      {activeAlerts.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Active Alerts ({activeAlerts.length})</h3>
      ...
      {resolvedAlerts.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Resolved Alerts ({resolvedAlerts.length})</h3>
